...
export default function CompanyEmployeesScreen() {
  ...
  const totalEmployeeCount = groups.reduce((sum, g) => sum + g.employees.length, 0);

  return (
    <View style={s.root}>
      <View style={[s.header, { paddingTop: insets.top + 16 }]}>
        <View>
          <Text style={s.headerTitle}>회사인원</Text>
          <Text style={s.headerSub}>부서 {groups.length}개 · 총 {totalEmployeeCount}명</Text>
        </View>
      </View>

      {loading ? (
        <View style={s.loadingWrap}>
          <ActivityIndicator size="small" color={C.companyIndigo} />
        </View>
      ) : (
        <ScrollView
          style={s.list}
...
const s = StyleSheet.create({
  ...
  list: { flex: 1 },
  listContent: { paddingHorizontal: 20, paddingTop: 12, gap: 20 },
