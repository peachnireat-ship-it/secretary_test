...
-- 감싸 필요한 컬럼(id, name, email, team, role)만 반환한다. contact는 여전히 비공개 정보이므로
-- 절대 포함하지 않는다. discoverable=true(옵트인)로 설정한 계정만 대상이며, 검색어 없이는 결과를
-- 반환하지 않는다(전체 덤프 방지), 호출자 자신은 제외, 최대 20건. 자세한 배경은
